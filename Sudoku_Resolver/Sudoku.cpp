...
Sudoku::comprobar(int const fila, int const columna, int const n) {
   //comprueba si n es un buen n�mero para introducir en la posicion fila columna
...
Sudoku::esResoluble()
{
   ...
   temp->SetSSComent(false); // buscamos una soluci�n sin comentarios
   temp->SetSSRand(false);   // buscamos una soluci�n. No importa cu�l.
   bOk = temp->resolver();
   delete temp;
   return bOk;
}
...
Sudoku::descarte(int const fila, int const columna) {
   ...
   case NOT_POSSIBLE_NUMBER: //La casilla est� vac�a y no se puede poner ning�n n�mero
...
Sudoku::posibleNumeroEn(int const fila, int const columna, int& candidato) {
   if (!EMPTY(m_tablero[fila][columna])) {
      // No tiene sentido comprobar los posibles n�meros en una casilla no vac�a
   ...
   for (int i = 1; i <= 9 && cuenta < 2; i++) {
      // Si puedo poner m�s de un n�mero, salgo del bucle
...
int
Sudoku::descartar() {
   // Rellenar las casillas en las que solo pueda ir un n�mero
   bool exito = false;
   bool puesto;
      ...
      for (int i = 0; i < 9; i++) {
         for (int j = 0; j < 9; j++) { // bucle doble para recorrer todo el tablero
            ...
            case NOT_POSSIBLE_NUMBER: //cout << "Error al descartar.";
               ...
               return NOT_POSSIBLE_NUMBER;
               //caso en el que haya una casilla vac�a que no acepta ning�n n�mero
            case ONLY_ONE_POSSIBLE:
...
Sudoku::trinidad() {
   ...
   for ( int i = 0; i < 9; i++ ) {
      // necesito que el cuadrante no est� completo para llamar a la trinidad
      emptyInCuad(i, nEmpty);
      if ( nEmpty > 0 ) {
         Trinidad tr( this, i );
         if ( tr.ponUnNumero() ) {
            // No me permito que ponga m�s de una posici�n.
...
void actualizaLista(std::vector<int>& list, int objetivo) {
   ...
   std::vector<int>::iterator index; //el �ndice de una posici�n en un vector tiene un tipo propio
...
Sudoku::emptyInLine(int nLine, int &cont) {
   // Devuelve la cantidad de casillas vac�as que hay en la fila introducida
...
Sudoku::emptyInCol(int nCol, int &cont){
   // Devuelve la cantidad de casillas vac�as que hay en la columna introducida
...
Sudoku::emptyInCuad(int nCuad, int &cont) {
   // Devuelve la cantidad de casillas vac�as que hay en el cuadrante introducido
